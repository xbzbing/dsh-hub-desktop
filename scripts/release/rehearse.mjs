#!/usr/bin/env node
/**
 * source-only GitHub Release 发布演练。
 *
 * 当前公开发布只包含 tag、Draft Release、发布说明以及 GitHub 自动生成的源码归档。
 * 本地打包命令仍可用于开发与受控验证，但其产物不得作为本次公开 Release 的资产上传。
 */
...
console.log(`\n=== DSH Hub source-only 发布演练（版本 ${version}，标签 ${tag}）===\n`)
...
if (notes) {
  ...
  await check('发布说明明确为 source-only 且不声明二进制资产', () => {
    assert(notes.distribution === 'source-only', `发布说明分发方式不是 source-only：${notes.distribution}`)
...
console.log('\n  不上传 dist/、.app、.dmg、.zip、.exe、latest-*.yml 或 SHA256SUMS.txt。')
