...
const NON_RENDERER_COPY_DEBT_USER_VISIBLE: readonly DebtEntry[] = [
  ...
  debt('src/main/ipc/register.ts', "throw new InstanceStoreError('invalid-input', '只有本机实例支持重启')"),
  debt('src/main/ipc/register.ts', "throw new InstanceStoreError('invalid-state', '实例未在运行，无法重启')"),
  debt('src/main/ipc/register.ts', "throw new InstanceStoreError('invalid-state', '外部接管的 dsh 进程归用户所有，不能由 hub 重启')"),
